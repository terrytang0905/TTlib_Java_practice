package com.newroad.util.practice.algorithm.sorting;
...
public enum SortType {
	...
	MERGE(new Sortable() {
		...
		private <T extends Comparable<T>> void sort(T[] array, int lo, int hi,
				boolean ascend) {
			// OPTIMIZE ONE
		...
		private <T extends Comparable<T>> void merge(T[] array, int lo,
				int mid, int hi, boolean ascend) {
			// OPTIMIZE TWO
			// if it is already in right order, skip this merge
			// since there's no need to do so
			int leftEndCompareToRigthStart = array[mid]
					.compareTo(array[mid + 1]);
			if (leftEndCompareToRigthStart == 0
					|| leftEndCompareToRigthStart < 0 == ascend) {
				return;
			}
	...
	QUICK(new Sortable() {
		...
		private <T extends Comparable<T>> int partition(T[] array, int lo,
	...
	HEAP(new Sortable() {
		...
		private <T extends Comparable<T>> void sink(T[] array, int nodeIdx,
				int heapSize, boolean ascend) {
			while (2 * nodeIdx + 1 < heapSize) {
				...
				if (childIdx + 1 < heapSize) {
					int childrenCompare = array[childIdx]
							.compareTo(array[childIdx + 1]);
					if (childrenCompare != 0 && childrenCompare < 0 == ascend) {
						childIdx++;
					}
				}

				int parentChildCompare = array[nodeIdx]
						.compareTo(array[childIdx]);
