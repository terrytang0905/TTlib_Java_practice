package com.ttlib.utilities.stringhandler;
...
public class OccurrenceAlgorithm {
	...
	public void indexSentences(String[] inputSentence, String regex,
	...
	private void indexString(String[] stringarray) {
	...
	public void showOccurence() {
		if (wordsMap.size() > 0) {
			Iterator<Entry<String, AtomicInteger>> iterword = wordsMap
					.entrySet().iterator();
			while (iterword.hasNext()) {
				Entry<String, AtomicInteger> wordEntry = iterword.next();
				System.out.println("'" + wordEntry.getKey() + "' is "
						+ wordEntry.getValue());
			}
		} else {
